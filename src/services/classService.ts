...
export const classService = {
  async getClasses(filters?: ClassFilters): Promise<Class[]> {
    const response = await api.get("/students/api/classes/", { params: filters });
  ...
  async updateClass(id: string, data: Partial<Class>): Promise<Class | null> {
    const response = await api.put(`/students/api/classes/${id}/`, data);
    return response.data;
  },
  ...
  async getStreams(filters?: StreamFilters): Promise<Stream[]> {
    const response = await api.get("/students/api/streams/", { params: filters });
    return response.data;
  },
  ...
  async updateStream(id: string, data: Partial<Stream>): Promise<Stream | null> {
    const response = await api.put(`/students/api/streams/${id}/`, data);
    return response.data;
  },
