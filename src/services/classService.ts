...
export const classService = {
  async getClasses(filters?: ClassFilters): Promise<Class[]> {
    ...
    return classesData.map(cls => {
      const classStreams = streams.filter((s: any) => {
        const assignedClassId = s.class_assigned_id ?? s.class_assigned;
        return String(assignedClassId) === String(cls.id);
      });
  ...
  async getStreams(filters?: StreamFilters): Promise<Stream[]> {
    ...
    const [streamsResult, classesResult] = await Promise.all([
      supabase
        .from('streams')
        .select('*')
        .eq('school_id', schoolId),
    ...
    const { data: streamsData, error } = streamsResult;
    
    if (error) throw error;
    
    if (!streamsData || streamsData.length === 0) return [];

    const classMap = new Map((classesResult.data || []).map((c: any) => [String(c.id), c]));

    const filteredByClass = filters?.class_id
      ? streamsData.filter((stream: any) => {
          const assignedClassId = stream.class_assigned_id ?? stream.class_assigned;
          return String(assignedClassId) === String(filters.class_id);
        })
      : streamsData;
    ...
    const streamsWithEnrollment = filteredByClass.map((stream: any) => {
      ...
      const assignedClassId = stream.class_assigned_id ?? stream.class_assigned;
      const classInfo = classMap.get(String(assignedClassId));
      
      return {
        ...stream,
        class_assigned: String(assignedClassId || ''),
        class_name: classInfo?.name || '',
        current_enrollment: enrolledStudents.length,
        grade_level: classInfo?.grade_level || 0
  ...
  async getStream(id: string): Promise<Stream | null> {
    ...
    const assignedClassId = (data as any).class_assigned_id ?? (data as any).class_assigned;
