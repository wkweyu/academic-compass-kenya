...
export default function PayrollReportsTab({ runs }: PayrollReportsTabProps) {
  ...
  const exportCSV = () => {
    ...
    if (reportType === 'summary') {
      headers = ['Emp No', 'Name', 'Department', 'Basic', 'Allowances', 'Gross', 'PAYE', 'NHIF', 'NSSF', 'Loans', 'Other Ded', 'Total Ded', 'Net Pay'];
      rows = entries.map(e => [
      ...
      headers = ['Emp No', 'Name', 'Gross', 'PAYE', 'NHIF', 'NSSF', 'Total Statutory'];
      rows = entries.map(e => [
        ...
        (Number(e.paye_deduction) + Number(e.nhif_deduction) + Number(e.nssf_deduction)).toFixed(2),
      ]);
    } else {
      ...
      <CardContent>
        {!selectedRunId ? (
          ...
          <Table>
            ...
            <TableBody>
              {entries.map(e => {
                const statutory = Number(e.paye_deduction) + Number(e.nhif_deduction) + Number(e.nssf_deduction);
                return (
                  <TableRow key={e.id}>
              ...
              <TableRow className="bg-muted/50 font-bold">
                ...
                <TableCell className="text-right">{formatCurrency(totalNHIF)}</TableCell>
                <TableCell className="text-right">{formatCurrency(totalNSSF)}</TableCell>
                <TableCell className="text-right">{formatCurrency(totalPAYE + totalNHIF + totalNSSF)}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
        ) : (
